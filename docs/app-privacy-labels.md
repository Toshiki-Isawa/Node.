...
|------|-----------------|----------------|
| **v1.0（現行・App Store 初版）** | `cloudSyncEnabled = false` / `timelapseEnabled = false` | **データ収集なし**（端末内のみ・広告なし） |
...
v1.0 では Supabase / PostHog / StoreKit の SDK は同梱されていても **呼び出されません**。AdMob SDK は v1.0 で完全に撤去済み（旧設計から廃止）。申告は **v1.0 の実際の挙動** に合わせてください。
...
v1.0 では観測記録は端末内（SwiftData）のみ。当方サーバーへの送信、サインイン、課金、広告 SDK の呼び出しはありません。タイムラプス機能はエントリポイントごと封鎖されています（v1.0.1 Pack で開放予定）。
...
- [x] v1.0 専用プライバシーポリシー（クラウド・課金・広告の条項なし。広告 / ATT / SKAdNetwork 関連の記述は v1.0 仕様外として削除済み）
...
Archive / Conservatory 利用時に Supabase / R2 へデータが送信されます。**広告 SDK は v1.1 でも利用しない**（Pack / Sub のいずれも広告なし）。
...
| AdMob | **撤去済み** | **撤去済み** | **撤去済み** |
| PostHog | 未使用 | アプリ内挙動の集計（任意） | アプリ内挙動の集計（任意） |
